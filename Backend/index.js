...
app.post("/", async (req, res) => {
    try {
        ...
        const { name, email, Phone, password, role, orders, testimonials } = req.body;
...
app.post('/courses', async (req, res) => {
    try {
        ...
        if (!title || !description || !price || !originalPrice) {
            return res.status(400).json({
                success: false,
                message: 'Missing required fields: title, description, price, and originalPrice are required'
            });
        }
